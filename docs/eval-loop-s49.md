...
| — | Promotion = eval accuracy vs champion. **Rule changed in s51 (review of this PR):** one-tailed exact McNemar test on paired case outcomes, promote iff `p <= alpha` (0.05); ties and small packs HOLD (≥5 discordant pairs, all challenger wins, needed at 0.05). Was `pass_rate >= champion AND no pass→fail flips`. `scripts/mlflow_registry.py promotion_verdict`. |
...
4. **Pass rule.** `passed = (g1.score is None or g1.score >= 0.8) and (g5 is None or g5.passed)` — D2's "Pass = G1 + G5", implemented in `score_case`. **`g3_format` was dropped from the gate** (W-C, 2026-09-10): it lints the in-process report object that s46 reduced to a by-product of the deck, and the first s49 run failed a correct three-slide deck on `summary is empty`. Its issues are still scored, persisted and shown — they just no longer decide. The judge label is recorded, displayed, and **does not gate** while `len(cases) < HOLDOUT_MIN_CASES`; `judge_gates(len(cases))` in `scripts/eval_run.py` is the one-line switch when the pack reaches 10.
...
| `make promote` | under the rule of the day, `pass_rate >= champion AND no pass->fail flips` → 1.0 vs 1.0, flips [], **PROMOTED v18 → v19** (re-run under the s51 McNemar rule on 2026-09-14: `discordant 0, p=1.0` → **HOLD** — the same tie no longer promotes); `@challenger` cleared (after fixing `mlflow_client.delete_alias`, which MLflow 3.x had been rejecting silently); `app.promotions` row written |
