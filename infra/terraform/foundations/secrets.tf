...
# ---- s35: the MCP server's own service key ---------------------------------
# Deliberately NOT generated by Terraform. A dpk_ key only exists once, at mint
# time, and its secret half is never stored anywhere — only a SHA-256. Having
# Terraform generate one would either put the plaintext in state or invent a
# second minting path that bypasses the app's own hashing. So: mint it through
# the admin API (POST /admin/service-accounts with surface="mcp", or the
# Settings tab), then push it here:
#
#   aws secretsmanager put-secret-value --profile data-qa \
#     --secret-id data-qa/mcp-service-key --secret-string 'dpk_...'
#
# Until then the MCP server starts fine and every tool reports that it has no
# key, rather than half-working.
resource "aws_secretsmanager_secret" "mcp_service_key" {
  name        = "${local.name}/mcp-service-key"
  description = "MCP_SERVICE_KEY — the MCP server's dpk_ key. Minted in-app, set via CLI."
}

resource "aws_secretsmanager_secret_version" "mcp_service_key" {
  secret_id     = aws_secretsmanager_secret.mcp_service_key.id
  secret_string = "REPLACE_ME_VIA_CLI"
