...
def _system_prompt() -> str:
    return (
        ...
        "returns {'value': float, 'month': 'YYYY-MM'}; skills.growth_rate(...) returns "
        "{group: ...} as soon as group_col is passed. Index before you format or do "
        'arithmetic: use f"{lv[\'value\']:,.0f}", never f"{lv:,.0f}" (that raises '
        "`unsupported format string passed to dict.__format__`). skills.top_growth(...) "
        "always returns a DataFrame [<group_col>, 'growth_pct'] — feed it straight to "
        "comparison_chart rather than indexing it like a mapping. A skill can also "
        "return None when there is not enough history or the base is zero — guard "
        "before formatting.\n"
